...
void CardSet::creatWidget()
{
	if(pwidget!=NULL) return ;
	pwidget=new CardSetWidget(this);
}
...
void CardSet::clear()
{
	...
	for(list<Card*>::iterator it=tcardSet.begin();it!=tcardSet.end();++it)
	{
		erase(*it);
	}
}
...
void CardSet::erase(Card* pcard)
{
	...
	if(pwidget!=NULL)
	{
		pwidget->w_glayout->removeWidget((QWidget*)pcard->pwidget);
		pwidget->_update();
	}
	if(pcard->pwidget!=NULL)
	{
		pcard->pwidget->hide();
	}
}
