...
Game::Game(QObject *parent):QObject(parent)
{
	...
	qDebug()<<"enter the path of first player's deck";//<<flush;
	qin>>deckpath[0];

	qDebug()<<"enter the path of second player's deck";//<<flush;
	qin>>deckpath[1];
...
void Game::startRound(int roundNum, int startTeam)
{
	...
	for(int team=0;team<2;++team)
	{
		...
		for(int i=0;i!=swapNum;++i)
		{
			...
			{
				qDebug()<<"Leader cann't be swaped, this operation will not be executed";
				continue;
			}
...
void Game::endRound(int roundNum)
{
	...
	system("cls");
	qDebug()<<"Round "<<roundNum<<" end "<<endl
		   <<"player 0: "<<score[0][roundNum]<<endl
		  <<"player 1: "<<score[1][roundNum]<<endl;
...
void Game::endGame()
{
	...
	if(winRound[0]>winRound[1]) qDebug()<<"player 0 win"<<endl;
	if(winRound[0]<winRound[1])	qDebug()<<"player 1 win"<<endl;
	if(winRound[0]==winRound[1]) qDebug()<<"Tie"<<endl;

	for(int k=0;k!=MAX_ROUND_NUM;++k)
	{
		qDebug()<<score[0][k]<<" : "<<score[1][k]<<endl;
	}
